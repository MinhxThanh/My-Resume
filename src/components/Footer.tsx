...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Footer appears when scrolled past 400px
      setIsScrolled(scrollY > 400);
      // Back to top button appears much earlier - after just 150px
  ...
  return (
    <>
      {/* Scroll to Top Button - Always rendered but visibility controlled */}
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        onClick={scrollToTop}
        className="fixed bottom-6 right-6 p-3 bg-gradient-to-r from-accent to-secondary hover:from-secondary hover:to-accent text-black rounded-full shadow-lg hover:shadow-xl transition-all duration-300 z-[80] border border-white/20"
        style={{
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-8">
              ...
              >
                ...
                <div className="space-y-2 flex flex-col items-center">
                  {['Home', 'About', 'Experience', 'Skills', 'Projects', 'Certifications', 'Languages', 'Interests', 'Contact'].map((link) => (
                    <motion.button
                      key={link}
                      whileHover={{ x: 5 }}
